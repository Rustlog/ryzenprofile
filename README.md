# RyzenProfile – runit/systemd supervisor and daemon that runs ryzenadj exactly once per change: kills old worker, spawns new one on power source or profile switch

## Why this exists?
- I dual-boot Void (daily) + Fedora (rarely) and wanted **one** tool that just works everywhere.
- Existing solutions are either systemd-only or ugly polling hacks.
- 4 days later -> this script. Set up once, forget forever.
